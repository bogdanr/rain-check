...
echo "== 1/20 collect archives (Tracks A and B, ERA5) =="
S collect_archive.py all
S collect_archive.py previous_runs icon_eu

echo "== 2/20 station observations (daily, GHCN) =="
S observations.py

echo "== 3/20 station observations (hourly present weather, NOAA ISD) =="
S observations_hourly.py

echo "== 4/20 build verification tables =="
S build_dataset.py

echo "== 5/20 validate the join (fails loudly on misalignment) =="
S validate_join.py

echo "== 6/20 analysis and robustness =="
S analyze.py
S robustness.py

echo "== 7/20 hourly track, derived-probability events, external benchmarks =="
S hourly.py
S events.py
S benchmarks.py

echo "== 8/20 European capitals: coverage probe, then the multi-city run =="
S probe_capitals.py
...
echo "== 9/20 forecast provenance audit, then the like-for-like ranking =="
...
echo "== 10/20 beyond the capitals: probe every city with a usable gauge =="
...
echo "== 11/20 multi-provider league: probe coverage, then collect =="
...
echo "== 12/20 per-provider verification, league table, robustness =="
...
echo "== 13/20 decision value, CRPS/ROC/sharpness, baselines (Tasks 18, 22-24) =="
...
echo "== 14/20 served vs member-derived probability (Task 21, triangulation) =="
...
echo "== 15/20 paired significance and FDR control (Tasks 25-26) =="
...
echo "== 16/20 physics against machine learning (Task 30) =="
...
echo "== 17/20 window homogeneity and change points (Tasks 17, 19; G6) =="
...
echo "== 18/20 how much of the verdict is where the bucket stands? (G3; Tasks 13-14) =="
...
echo "== 19/20 which day does the gauge mean? (Task 6, G11) =="
...
echo "== 20/20 build the HTML report =="
