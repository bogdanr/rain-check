...
echo "== 1/27 collect archives (Tracks A and B, ERA5) =="
S collect_archive.py all
S collect_archive.py previous_runs icon_eu

echo "== 2/27 station observations (daily, GHCN) =="
S observations.py

echo "== 3/27 station observations (hourly present weather, NOAA ISD) =="
S observations_hourly.py

echo "== 4/27 build verification tables =="
S build_dataset.py

echo "== 5/27 validate the join (fails loudly on misalignment) =="
S validate_join.py

echo "== 6/27 analysis and robustness =="
S analyze.py
S robustness.py

echo "== 7/27 hourly track, derived-probability events, external benchmarks =="
S hourly.py
S events.py
S benchmarks.py

echo "== 8/27 European capitals: coverage probe, then the multi-city run =="
S probe_capitals.py
...
echo "== 9/27 forecast provenance audit, then the like-for-like ranking =="
...
echo "== 11/27 multi-provider league: probe coverage, then collect =="
...
echo "== 13/27 decision value, CRPS/ROC/sharpness, baselines (Tasks 18, 22-24) =="
...
echo "== 14/27 served vs member-derived probability (Task 21, triangulation) =="
...
echo "== 15/27 paired significance and FDR control (Tasks 25-26) =="
...
echo "== 16/27 physics against machine learning (Task 30) =="
...
echo "== 17/27 window homogeneity and change points (Tasks 17, 19; G6) =="
...
echo "== 18/27 how much of the verdict is where the bucket stands? (G3; Tasks 13-14) =="
...
echo "== 19/27 which day does the gauge mean? (Task 6, G11) =="
...
echo "== 20/27 what sample can this study have? (Tasks 10, 16; G1, G14, G20) =="
...
echo "== 21/27 can ISD reach where GHCN cannot? (Task 11; G1) =="
...
echo "== 22/27 is the forecast worse where people are poorer? (Tasks 28, 29; D7) =="
...
echo "== 23/27 can a satellite stand in for the gauge that is not there? (Task 12; G1) =="
# E30 left G1 split: Asia is engineering, Africa and South America need a
# non-gauge truth source or they stay outside every verification study. This
# stage reads two CHIRPS-family products -- one that blends station reports
# and one that does not -- against 649 GHCN gauges inside the 50S-50N band,
# aligning calendars first because E24 priced a one-day error at 0.07 Brier.
# Downloads 400 global daily rasters; cached after the first run.
run_fresh S satellite_truth.py -- \
  data/processed/satellite_truth_daily.parquet \
  data/processed/satellite_truth_scores.parquet

echo "== 24/27 what predicts how well a city is forecast? (Task 27; G4) =="
# Every other stage measures how badly the forecast does and who it fails.
# This one asks why, and answers in the negative: seven city properties are
# regressed on the skill, reliability and decision-value scores, with
# countries as the resampling unit because E28 measured the design effect at
# 4.3. Validation is leave-one-country-out, so a fit the panel's own countries
# explain cannot pass as a mechanism.
run_fresh S explain.py -- \
  data/processed/explain_coefficients.parquet \
  data/processed/explain_cv.parquet

echo "== 25/27 is the dry bias the forecast, or the reference? (Task 20b) =="
# E13 measured the served probability as drier than the GEFS members that
# ought to support it, and a reviewer's first question is whether that is a
# statement about NOAA rather than about the forecast. This stage puts a
# second, independent centre - ECMWF IFS ENS, 51 members - on the identical
# city-days, at a matched 3-hourly ladder so E23's boundary artefact cannot
# leak in, with the member count subsampled to GEFS's 31 as a control.
run_fresh S reference_centre.py -- \
  data/processed/reference_centre_panel.parquet \
  data/processed/reference_centre_headline.parquet \
  data/processed/reference_centre_controls.parquet

echo "== 26/27 build the HTML report =="
run_fresh S report.py -- dist/index.html

echo "== 27/27 what may be republished, and under what terms? (Task 9; G15) =="
# Runs last because it audits the rendered pages as well as the tables: every
# input licence read from its publisher, every published file traced to the
# models whose values reach it, and the one share-alike input followed to the
# files that inherit it. Fails the build if a page shows Open-Meteo data
# without the link its licence requires, or if a headline table ever comes to
# carry a Met Office series.
run_fresh S provenance.py -- \
  data/processed/provenance_sources.parquet \
  data/processed/provenance_artefacts.parquet \
  NOTICE.md
