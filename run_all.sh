...
echo "== 1/26 collect archives (Tracks A and B, ERA5) =="
S collect_archive.py all
S collect_archive.py previous_runs icon_eu

echo "== 2/26 station observations (daily, GHCN) =="
S observations.py

echo "== 3/26 station observations (hourly present weather, NOAA ISD) =="
S observations_hourly.py

echo "== 4/26 build verification tables =="
S build_dataset.py

echo "== 5/26 validate the join (fails loudly on misalignment) =="
S validate_join.py

echo "== 6/26 analysis and robustness =="
S analyze.py
S robustness.py

echo "== 7/26 hourly track, derived-probability events, external benchmarks =="
S hourly.py
S events.py
S benchmarks.py

echo "== 8/26 European capitals: coverage probe, then the multi-city run =="
S probe_capitals.py
...
echo "== 9/26 forecast provenance audit, then the like-for-like ranking =="
...
echo "== 10/26 beyond the capitals: probe every city with a usable gauge =="
...
echo "== 11/26 multi-provider league: probe coverage, then collect =="
...
echo "== 12/26 per-provider verification, league table, robustness =="
...
echo "== 13/26 decision value, CRPS/ROC/sharpness, baselines (Tasks 18, 22-24) =="
...
echo "== 14/26 served vs member-derived probability (Task 21, triangulation) =="
...
echo "== 16/26 physics against machine learning (Task 30) =="
...
echo "== 17/26 window homogeneity and change points (Tasks 17, 19; G6) =="
...
echo "== 18/26 how much of the verdict is where the bucket stands? (G3; Tasks 13-14) =="
...
echo "== 19/26 which day does the gauge mean? (Task 6, G11) =="
...
echo "== 20/26 what sample can this study have? (Tasks 10, 16; G1, G14, G20) =="
...
echo "== 21/26 can ISD reach where GHCN cannot? (Task 11; G1) =="
...
echo "== 22/26 is the forecast worse where people are poorer? (Tasks 28, 29; D7) =="
...
echo "== 23/26 can a satellite stand in for the gauge that is not there? (Task 12; G1) =="
...
echo "== 24/26 what predicts how well a city is forecast? (Task 27; G4) =="
...
echo "== 25/26 build the HTML report =="
run_fresh S report.py -- dist/index.html

echo "== 26/26 what may be republished, and under what terms? (Task 9; G15) =="
