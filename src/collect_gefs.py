"""Extraction of NOAA GEFS ensemble members at city points (plan Task 20a).
...
vendor retention window and can never be rebuilt, this archive is retrospective
and gapless back to 2020-10-01, so this collector is a batch job that can be
re-run, resumed and repointed at a different city set at will.

The cost model, which is the whole design
-----------------------------------------
The archive's chunk is `[1 init, 31 members, 64 leads, 17 lat, 16 lon]`. One
chunk therefore carries every member and every lead out to 189 h for a
4.25 deg x 4 deg tile. The unit of work is the (tile, init) pair, not the
city: all cities inside a tile are served by one read, which is why the
collector groups by tile first and only then indexes out the cities. At 2,000
cities that is 480 tiles rather than 2,000 point queries.

A correction to the feasibility report's projection, measured here rather than
assumed. Local day k ends 24(k+1) - utc_offset hours after the 00Z init, so:
...
UTC+3 and east; elsewhere day 7 doubles the transfer. The run prints the
measured chunks and megabytes per (tile, init) so the scale-up projection is
re-derived from what actually happened, not quoted.

Output layout
-------------
    data/raw/gefs/<YYYY-MM>/gefs_<city>.parquet
    data/raw/gefs/<YYYY-MM>/manifest.json
...
kg m-2 s-1 over the preceding 3-hour step - not as accumulations or
probabilities. The raw quantity is the archive of record here; thresholds,
day conventions and boundary handling are all decisions that will be revisited
(see src/ensemble_pop.py), and baking any of them into the stored data would
force a re-download to change one's mind.

Usage:
    python src/collect_gefs.py plan [capitals|cities]   # cost, fetches nothing
    python src/collect_gefs.py collect [capitals|cities] [--start=] [--end=]
    python src/collect_gefs.py status
    python src/collect_gefs.py sanity                   # the Bucharest check
"""
...
from gefs_archive import grid_of, group_by_tile, open_dataset, rx_bytes

# The store is chunked at 64 leads; reading 65 indices would cost two chunks
# for one value. Lead index requirements are therefore rounded UP to a
# multiple of this, and the number of chunks that implies is what gets
# reported as the true cost.
LEAD_CHUNK = 64
...
def out_path(city_name: str, month: str):
    return GEFS_RAW / month / f"gefs_{_slug(city_name)}.parquet"


# --------------------------------------------------------------------------
# Planning
# --------------------------------------------------------------------------
def plan(cities: dict[str, City], inits: np.ndarray, lead_days=LEAD_DAYS) -> dict:
    ...
    which is enough to push local day 7 across the 189 h chunk boundary: a
    depth planned in July and applied in January would silently truncate the
    last day of every European city for half the record.
    """
    ...
    for tile, members in tiles.items():
        ...
        n_lead = int(np.ceil((need + 1) / LEAD_CHUNK) * LEAD_CHUNK)
        per_tile[tile] = {"cities": members, "need_index": need,
                          "n_lead": n_lead, "chunks": n_lead // LEAD_CHUNK}
    total_chunks = sum(t["chunks"] for t in per_tile.values()) * len(inits)
    return {"grid": grid, "tiles": per_tile, "inits": inits,
            "total_chunks": total_chunks}


def tile_need_index(cities: dict[str, City], members, inits, lead_days) -> int:
...
def _tz_need(tz: str, inits, lead_days: tuple) -> int:
    key = (tz, len(inits), str(inits[0]), str(inits[-1]), lead_days)
    if key not in _TZ_NEED:
        _TZ_NEED[key] = max(required_lead_index(pd.Timestamp(t), tz, lead_days)
                            for t in inits)
    return _TZ_NEED[key]


def _print_plan(p: dict, cities: dict[str, City]) -> None:
    one = sum(1 for t in p["tiles"].values() if t["chunks"] == 1)
    two = len(p["tiles"]) - one
    ...
    print(f"lead chunks/tile  {one} tiles need 1, {two} need 2 "
          f"(2 = a time zone west of UTC+3 needs local day 7)")
    print(f"chunk reads       {p['total_chunks']:,}")
    print(f"projected transfer ~{p['total_chunks'] * 0.226 / 1024:.1f} GB "
          f"at the feasibility report's 0.226 MB per chunk")


# --------------------------------------------------------------------------
# Completion test
# --------------------------------------------------------------------------
def _month_complete(city_name: str, month: str, want_inits: list[pd.Timestamp],
                    need_index: int) -> bool:
    path = out_path(city_name, month)
    if not path.exists():
        return False
    try:
...
# Collection
# --------------------------------------------------------------------------
def collect(city_set: str = "capitals", start: str = GEFS_WINDOW_START,
            end: str = GEFS_WINDOW_END, lead_days=LEAD_DAYS,
            *, force: bool = False, max_months: int | None = None) -> None:
    cities = load_capitals() if city_set == "capitals" else load_cities()
    ds = open_dataset()
    var = ds[GEFS_VARIABLE]

    all_inits = pd.to_datetime(ds.init_time.values)
    sel = (all_inits >= pd.Timestamp(start)) & (all_inits <= pd.Timestamp(end))
    inits = all_inits[sel]
    if len(inits) == 0:
        raise SystemExit(f"no initialisations in {start}..{end}")
    # Gaplessness is a stated property of this archive (D1/G6 depend on it),
    ...
    p = plan(cities, inits.values, lead_days)
    _print_plan(p, cities)
    init_index = {pd.Timestamp(t): i for i, t in
                  enumerate(pd.to_datetime(ds.init_time.values))}
    ...
    for month in months:
        ...
        for tile, info in p["tiles"].items():
            todo = [(n, ila, ilo) for n, ila, ilo in info["cities"]
                    if force or not _month_complete(n, month, m_inits,
                                                    info["need_index"])]
            if todo:
        ...
        for tile, (info, todo) in sorted(pending.items()):
            ...
            def read(t_init, n_lead=n_lead, lat_sl=lat_sl, lon_sl=lon_sl):
                ...
                return var.isel(init_time=init_index[t_init],
                                ...
                                latitude=lat_sl, longitude=lon_sl).values
        ...
        _manifest(month, m_inits, pending, n_chunk_m, rx_m, time.time() - t0,
                  lead_days)
    ...
    if chunks_read:
        print(f"measured cost: {rx / chunks_read:.3f} MB and "
              ...
              f"(feasibility report: 0.226 MB, ~0.09 s)")
...
def _manifest(month: str, m_inits, pending, n_chunks, mb, secs, lead_days
              ) -> None:
    path = GEFS_RAW / month / "manifest.json"
    payload = {
        "month": month,
        "n_inits": len(m_inits),
        ...
        "variable": GEFS_VARIABLE,
        "units": "kg m-2 s-1 (mean rate over the preceding 3 h step)",
...
# --------------------------------------------------------------------------
def status() -> None:
    files = sorted(GEFS_RAW.glob("*/gefs_*.parquet"))
    if not files:
        print(f"no GEFS extraction yet under {GEFS_RAW}")
        return
    rows = [{"month": f.parent.name, "city": f.stem.replace("gefs_", ""),
    ...
    mans = sorted(GEFS_RAW.glob("*/manifest.json"))
    if mans:
...
def sanity() -> None:
    ...
    blk = ds[GEFS_VARIABLE].isel(init_time=it, lead_time=slice(0, 64),
...
if __name__ == "__main__":
    ...
    args = sys.argv[2:]
    city_set = next((a for a in args if not a.startswith("-")), "capitals")
    ...
    if cmd == "plan":
        ...
        sel = it[(it >= pd.Timestamp(kw.get("start", GEFS_WINDOW_START)))
                 & (it <= pd.Timestamp(kw.get("end", GEFS_WINDOW_END)))]
        _print_plan(plan(cs, sel.values), cs)
    elif cmd == "collect":
        collect(city_set, kw.get("start", GEFS_WINDOW_START),
                kw.get("end", GEFS_WINDOW_END), force="--force" in args,
