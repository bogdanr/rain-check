"""ISO 3166-1 alpha-2 code -> short English country name, for display only.

Kept in the repository rather than read from the World Bank cache that
`equity.py` downloads: the site build must not depend on a cache another stage
may not have written, and the World Bank's catalogue names ("Korea, Rep.",
"Congo, Dem. Rep.") read badly in a heading. Names follow common English usage.
`XK` (Kosovo) is a user-assigned code GeoNames uses.
"""
...
NAMES = {
    ...
    "HK": "Hong Kong", "HM": "Heard and McDonald Islands", "HN": "Honduras", "HR": "Croatia", "HT": "Haiti",
    "HU": "Hungary",
...
def country_name(code: str) -> str:
    """Display name for a code; unknown codes are returned unchanged."""
    return NAMES.get((code or "").upper(), code or "")


if __name__ == "__main__":
    assert len(NAMES) == 250, len(NAMES)
    assert country_name("jp") == "Japan" and country_name("ZZ") == "ZZ"
    assert country_name("") == ""
    print(f"country_names: {len(NAMES)} codes ok")
